...
# OR Lukasiewicz (no constraint)
class or_lukasiewicz_no_constraint(nn.Module):
    def __init__(self, alpha, num_predicates, use_slack=False):
        super().__init__()
        body_len = 1
        self.alpha=alpha
        self.weights = nn.Parameter(torch.zeros(body_len, 
                                    num_predicates).uniform_(0.0, 0.1))
        self.beta = nn.Parameter(torch.ones(1, body_len))
        
    def forward(self, x):
        beta, weights = self.get_params()      
        #lnn disjunction without constraints  
...
# AND Lukasiewicz (no constraint)
class and_lukasiewicz_no_constraint(nn.Module):
    def __init__(self, alpha, num_predicates, use_slack=False):
        super().__init__()
        body_len = 1
        self.alpha=alpha
    ...
    def forward(self, x):
        beta, weights = self.get_params()        
        #lnn conjunction without constraints
        ret = torch.clamp(-func.linear(1 - x, weights) + beta, 0, 1) 
        return ret, None
...
class and_lukasiewicz(nn.Module):
    def __init__(self, alpha, arity, with_slack, use_constraint=False):
        '''
